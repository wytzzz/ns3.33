...
Ns3QuicClientSession::Ns3QuicClientSession(
    std::unique_ptr<QuicConnection> connection_ptr, Visitor *owner,
    const QuicConfig &config, const ParsedQuicVersionVector &supported_versions,
    const QuicServerId &server_id, QuicCryptoClientConfig *crypto_config,
    Ns3QuicBackendBase *backend)
    : Ns3QuicSessionBase(connection_ptr.get(), owner, config,
                         supported_versions, 0),
      backend_(backend), connection_own_(std::move(connection_ptr)) {
  /*for (const ParsedQuicVersion& version : supported_versions) {
      QUIC_BUG_IF(Ns3QuicClientSession,version.handshake_protocol !=
  PROTOCOL_TLS1_3)
          << "QuicTransport requires TLS 1.3 handshake";
  }*/
  // std::cout<<"max uni:
  // "<<config.GetMaxUnidirectionalStreamsToSend()<<std::endl; std::cout<<"max
  // bi: "<<config.GetMaxBidirectionalStreamsToSend()<<std::endl;
  ...
  if (!endpoint_) {
    QUIC_LOG(ERROR) << "endpoint is null " << connection_own_.get() << " "
                    << connection();
...
QuicStream *
Ns3QuicClientSession::CreateIncomingStream(PendingStream * /*pending*/) {
  return nullptr;
}
...
void Ns3QuicClientSession::OnProofValid(
    const QuicCryptoClientConfig::CachedState &cached) {}
void Ns3QuicClientSession::OnProofVerifyDetailsAvailable(
