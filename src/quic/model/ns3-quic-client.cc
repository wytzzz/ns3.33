...
namespace quic {
static std::string
CongestionControlTypeToString(CongestionControlType cc_type) {
  switch (cc_type) {
  case kCubicBytes:
    return "CUBIC_BYTES";
  case kRenoBytes:
...
Ns3QuicClient::Ns3QuicClient(QuicSession::Visitor *owner,
    ...
    : Ns3QuicClient(owner, server_address, server_id, supported_versions,
                    QuicConfig(), std::move(proof_verifier), nullptr, engine,
                    backend, poller, cc_type) {}

Ns3QuicClient::Ns3QuicClient(
    QuicSession::Visitor *owner, QuicSocketAddress server_address,
    const QuicServerId &server_id,
    const ParsedQuicVersionVector &supported_versions, const QuicConfig &config,
    std::unique_ptr<ProofVerifier> proof_verifier,
    std::unique_ptr<SessionCache> session_cache, Ns3QuicAlarmEngine *engine,
    Ns3QuicBackendBase *backend, Ns3QuicPollServer *poller,
    CongestionControlType cc_type)
    : QuicClientBase(server_id, supported_versions, config,
      ...
      backend_(backend), owner_(owner), cc_type_(cc_type) {
...
bool Ns3QuicClient::GetBandwidth(QuicBandwidth &bandwidth) const {
  ...
  if (session && session->connection()) {
    const QuicSentPacketManager &manager =
