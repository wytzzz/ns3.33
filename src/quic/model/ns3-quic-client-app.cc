...
public:
  QuicClientAppImpl(QuicClientApp *outer, quic::BackendType backend_type,
                    quic::CongestionControlType cc);
  ~QuicClientAppImpl() override;
  void OnIncomingData(quic::QuicSocketAddress self_addr,
                      quic::QuicSocketAddress peer_addr, const char *buffer,
                      size_t sz);
  ...
  int WritePacket(const char *buffer, size_t buf_len,
  ...
  void
  OnWriteBlocked(quic::QuicBlockedWriterInterface *blocked_writer) override;
...
QuicClientApp::QuicClientApp(quic::BackendType backend_type,
                             const char *cc_name) {
  m_impl.reset(
      new QuicClientAppImpl(this, backend_type, GetCongestionType(cc_name)));
}
...
void QuicClientApp::RecvPacket(Ptr<Socket> socket) {
  ...
  // NS_LOG_INFO(this<<" QuicClientApp::recv "<<ns3_peer_sock_addr.GetIpv4()<<"
  // "<<length);
  Ns3QuicTag tag;
  ...
  if (m_impl) {
    ...
    InetSocketAddress ns3_peer_sock_addr =
        InetSocketAddress::ConvertFrom(remoteAddr);
    quic::QuicSocketAddress peer_addr = GetQuicSocketAddr(ns3_peer_sock_addr);
    NS_ASSERT((ns3_peer_sock_addr.GetIpv4() == m_peerIp) &&
              (ns3_peer_sock_addr.GetPort() == m_peerPort));
...
void QuicClientApp::SendToNetwork(const char *buffer, size_t sz) {
  ...
  // NS_LOG_INFO(this<<" QuicClientApp::send "<<remote.GetIpv4()<<"
  // "<<remote.GetPort()<<" "<<now.GetSeconds());
...
void QuicClientApp::SetRateTraceFuc(TraceOneFloatValue cb) { m_traceRate = cb; }
void QuicClientApp::SetCwndTraceFun(TraceOneIntValue cb) { m_traceCwnd = cb; }
...
QuicClientAppImpl::QuicClientAppImpl(QuicClientApp *outer,
                                     quic::BackendType backend_type,
                                     quic::CongestionControlType cc)
    : m_outer(outer), m_cc(cc) {
...
bool QuicClientAppImpl::CreateUDPSocketAndBind(
    quic::QuicSocketAddress server_address, quic::QuicIpAddress bind_to_address,
    int bind_to_port) {
  // NS_LOG_INFO(server_address.ToString()<<" "<<bind_to_address.ToString()<<"
  // "<<bind_to_port);
  return true;
}
...
int QuicClientAppImpl::WritePacket(
    const char *buffer, size_t buf_len, const quic::QuicIpAddress &self_address,
    const quic::QuicSocketAddress &peer_address) {
  m_outer->SendToNetwork(buffer, buf_len);
  return buf_len;
}

void QuicClientAppImpl::OnConnectionClosed(
    quic::QuicConnectionId server_connection_id, quic::QuicErrorCode error,
    const std::string &error_details, quic::ConnectionCloseSource source) {
  NS_LOG_ERROR("OnConnectionClosed " << error_details);
}
void QuicClientAppImpl::OnWriteBlocked(
    quic::QuicBlockedWriterInterface *blocked_writer) {}
void QuicClientAppImpl::OnRstStreamReceived(
    const quic::QuicRstStreamFrame &frame) {}
void QuicClientAppImpl::OnStopSendingReceived(
...
void QuicClientAppImpl::OnConnectionIdRetired(
    const quic::QuicConnectionId &server_connection_id) {}
...
void QuicClientAppImpl::InitialAndConnect(quic::QuicSocketAddress self_addr,
  ...
  quic::QuicServerId server_id(peer_addr.host().ToString(), peer_addr.port());
  m_quicClient.reset(new quic::Ns3QuicClient(
      this, peer_addr, server_id, versions, std::move(proof_verifier),
      m_backend->get_engine(), m_backend.get(), this, m_cc));
