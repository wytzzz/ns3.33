...
namespace quic {
Ns3ClientNetworkHelper::Ns3ClientNetworkHelper(Ns3QuicPollServer *poller,
...
bool Ns3ClientNetworkHelper::CreateUDPSocketAndBind(
    QuicSocketAddress server_address, QuicIpAddress bind_to_address,
    int bind_to_port) {
  bool ret = poller_->CreateUDPSocketAndBind(server_address, bind_to_address,
                                             bind_to_port);
  if (ret) {
...
void Ns3ClientNetworkHelper::ProcessPacket(
