...
TypeId Ns3QuicTag::GetTypeId(void) {
  static TypeId tid =
      TypeId("ns3::Ns3QuicTag")
          .SetParent<Tag>()
          .AddConstructor<Ns3QuicTag>()
          .AddAttribute("Time", "time stamp", EmptyAttributeValue(),
                        MakeUintegerAccessor(&Ns3QuicTag::GetTime),
                        MakeUintegerChecker<uint64_t>())
          .AddAttribute("Number", "sequence number", EmptyAttributeValue(),
                        MakeUintegerAccessor(&Ns3QuicTag::GetSequence),
                        MakeUintegerChecker<uint64_t>());
  return tid;
}
TypeId Ns3QuicTag::GetInstanceTypeId(void) const { return GetTypeId(); }
