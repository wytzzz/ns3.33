...
static inline int uuid_allocator() { return g_uuid++; }
...
void set_quic_goodput_interval(Time interval) { g_goodputInterval = interval; }
Ns3QuicClientTrace::Ns3QuicClientTrace() { m_uuid = uuid_allocator(); }
Ns3QuicClientTrace::~Ns3QuicClientTrace() { Close(); }

void Ns3QuicClientTrace::LogEnable(const std::string &prefix,
                                   QuicClientTraceType e) {
  char buf[FILENAME_MAX];
  memset(buf, 0, FILENAME_MAX);
  std::string name;
  if (0 == Ns3QuicTracePath.size()) {
    name = std::string(getcwd(buf, FILENAME_MAX)) + "/traces/" + prefix;
  } else {
...
Ns3QuicServerTrace::Ns3QuicServerTrace(std::string &prefix,
                                       QuicServerTraceType e) {
  char buf[FILENAME_MAX];
  memset(buf, 0, FILENAME_MAX);
  std::string name;
  if (0 == Ns3QuicTracePath.size()) {
...
void Ns3QuicServerTrace::OnOwd(int seq, int owd, int len) {
  if (m_owd.is_open() || m_goodput.is_open()) {
    ...
    m_readBytes += len;
    if (m_owd.is_open()) {
      m_owd << now.GetSeconds() << "\t" << seq << "\t" << owd << "\t" << len
            << std::endl;
      m_receiptTime = now;
    }
    LogGoodput(false);
...
void Ns3QuicServerTrace::LogLostPackets() {
  if (m_lost.is_open()) {
    ...
    m_lost << m_lostPackets << "\t" << m_largestSeq << "\t" << ratio
           << std::endl;
  }
}
  ...
  if (m_goodput.is_open()) {
    if (dtor) {
      if (m_receiptTime > m_lastRateTime) {
        ...
        float kbps =
            1.0 * (m_readBytes - m_lastRateBytes) * 8 / delta.GetMilliSeconds();
...
Ns3QuicServerTraceDispatcher::~Ns3QuicServerTraceDispatcher() { Shutdown(); }
...
void Ns3QuicServerTraceDispatcher::LogEnable(std::string &topo_id,
                                             QuicServerTraceType e) {
  m_topoId = topo_id;
  m_e = e;
  if (E_QS_UTIL && e) {
...
bool Ns3QuicServerTraceDispatcher::AddMonitorAddressPair(
    const Ns3QuicAddressPair &addr_pair) {
  bool success = false;
...
void Ns3QuicServerTraceDispatcher::OnOwd(const InetSocketAddress &c,
                                         const InetSocketAddress &s, int seq,
                                         int owd, int len) {
  Time now = Simulator::Now();
  m_readBytes += len;
  if (Time(0) == m_firstPacketTime) {
...
void Ns3QuicServerTraceDispatcher::OpenUtilFile(const std::string &topo_id) {
  ...
  std::string name;
  if (0 == Ns3QuicTracePath.size()) {
    name = std::string(getcwd(buf, FILENAME_MAX)) + "/traces/" + topo_id +
           "_util.txt";
  } else {
