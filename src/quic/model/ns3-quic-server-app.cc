...
void QuicServerApp::RecvPacket(Ptr<Socket> socket) {
  ...
  quic::QuicSocketAddress quic_self_sock_addr =
      GetQuicSocketAddr(ns3_self_sock_addr);

  Address remoteAddr;
  auto packet = socket->RecvFrom(remoteAddr);
  InetSocketAddress ns3_peer_sock_addr =
      InetSocketAddress::ConvertFrom(remoteAddr);
  quic::QuicSocketAddress quic_peer_sock_addr =
      GetQuicSocketAddr(ns3_peer_sock_addr);
  uint32_t length = packet->GetSize();
  // NS_LOG_INFO(this<<" QuicServerApp::recv "<<ns3_peer_sock_addr.GetIpv4()<<"
  // "<<length);
  Ns3QuicTag tag;
  ...
  if (m_impl) {
    m_impl->OnIncomingData(quic_self_sock_addr, quic_peer_sock_addr, buffer,
                           (size_t)length);
  }
...
void QuicServerApp::SendToNetwork(const char *buffer, size_t sz,
                                  const InetSocketAddress &dest) {
  Time now = Simulator::Now();
  Ptr<Packet> packet = Create<Packet>((const uint8_t *)buffer, sz);
  // NS_LOG_INFO(this<<" QuicServerApp::send "<<dest.GetIpv4()<<"
  // "<<dest.GetPort()<<" "<<now.GetSeconds());
...
QuicServerAppImpl::QuicServerAppImpl(QuicServerApp *outer,
                                     quic::BackendType type)
    : m_outer(outer) {
  m_clock = quic::Ns3QuicClock::Instance();
  m_engine.reset(new quic::Ns3QuicAlarmEngine(quic::Perspective::IS_SERVER));
  if (quic::BackendType::HELLO_UNIDI == type ||
      quic::BackendType::HELLO_BIDI == type) {
...
QuicServerAppImpl::~QuicServerAppImpl() { Stop(); }

int QuicServerAppImpl::WritePacket(
...
int QuicServerAppImpl::Start() {
  ...
  m_quicServer.reset(new quic::Ns3QuicServer(std::move(proof_source), engine,
                                             m_backend.get()));
