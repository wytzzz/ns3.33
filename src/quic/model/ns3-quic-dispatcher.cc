...
Ns3QuicDispatcher::Ns3QuicDispatcher(
    ...
    uint8_t expected_server_connection_id_length, Ns3QuicBackendBase *backend)
...
std::unique_ptr<QuicSession> Ns3QuicDispatcher::CreateQuicSession(
    ...
    const ParsedQuicVersion &version, absl::string_view /*sni*/) {
  // The QuicServerSessionBase takes ownership of |connection| below.
  std::unique_ptr<QuicConnection> connection;
  connection.reset(
      new QuicConnection(server_connection_id, self_address, peer_address,
                         helper(), alarm_factory(), writer(),
                         /* owns_writer= */ false, Perspective::IS_SERVER,
                         ParsedQuicVersionVector{version}));

  auto session = std::make_unique<Ns3QuicServerSession>(
      std::move(connection), this, config(), GetSupportedVersions(),
      crypto_config(), compressed_certs_cache(), backend_);
  session->Initialize();
  return session;
}
} // namespace quic
