...
Ns3TransportStream::Ns3TransportStream(QuicStreamId id, QuicSession *session)
...
bool Ns3TransportStream::Write(const char *data, size_t size, bool fin) {
  ...
  QuicUniqueBufferPtr buffer = MakeUniqueBuffer(
      session()->connection()->helper()->GetStreamSendBufferAllocator(), size);
  memcpy(buffer.get(), data, size);
  QuicMemSlice memslice(std::move(buffer), size);
  QuicConsumedData consumed = WriteMemSlices(QuicMemSliceSpan(&memslice), fin);
