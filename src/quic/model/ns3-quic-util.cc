...
int WallTimeMillis() { return WallTimeNowInUsec() / 1000; }
...
void set_quic_log_error_level() { set_quic_log_level(quiche::ERROR); }
...
bool set_quic_cert_key(const std::string &quic_cert_path) {
  ...
  std::string certificate_file_path =
      quic_cert_path + std::string("leaf_cert.pem");
...
std::string Ns3QuicAddressPair2String(const Ns3QuicAddressPair &addr_pair) {
  ...
  ret = client_ip + colon + client_port + undeline + server_ip + colon +
        server_port;
  return ret;
}
} // namespace ns3
